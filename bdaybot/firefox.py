...
def post_checker(post):
    happy_word_list = ['happy', 'hapy', 'hapi', 'appy', 'hpy', '#appy']
    birthday_word_list = ['birthday', 'bday', 'budday', 'b\'day', 'bdy', 'b\'day!']
    return any(word in post for word in happy_word_list) and any(word in post for word in birthday_word_list)
...
try:
    ...
    for x in range(4):
        try:
            ...
            time.sleep(5)
    # To search for the birthday posts, if it has not been loaded, it will scroll down the page, max 4 times.
    ...
    print("Total {} elements in posts...".format(len(posts)))
    ...
    print("Total {} posts found ".format(count))

    print("Successfully liked and wrote the comment on relevant posts: {} ".format(no_of_posts_liked))
